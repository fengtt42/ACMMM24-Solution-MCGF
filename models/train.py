...
if opt.iaa:
    ...
    if opt.multi_weather:
        ...
        image_datasets['drone'] = ImageFolder_iaa_multi_weather(os.path.join(data_dir, 'drone'),
                                                                transform=transform_iaa_drone_list,
                                                                iaa_transform=iaa_drone_transform,
                                                                iaa_weather_list=iaa_weather_list,
                                                                batchsize=opt.batchsize, shuffle=True, norm=opt.norm,
                                                                select=True)
    else:
        image_datasets['drone'] = ImageFolder_iaa_selectID(os.path.join(data_dir, 'drone'),
                                                           transform=transform_iaa_drone_list,
...
def calc_style_loss(target, input):
    ...
    for i in range(len(input)):
        ...
        style_loss = style_loss + (
                    mse_loss(input_mean, target_mean.detach()) + mse_loss(input_std, target_std.detach()))
    return style_loss
            ...
            for data, data2, data3, data4 in zip(dataloaders['satellite'], dataloaders['street'], dataloaders['drone'],
                ...
                if phase == 'val':
                    ...
                    if opt.views == 2:
                        ...
                        if opt.extra_Google:
                            if opt.norm == 'ada-ibn':
                                outputs, outputs2, outputs3, outputs4, sout_w, dout_w = model(inputs, inputs2, inputs3,
                                                                                              inputs4)
                            elif opt.norm == 'spade':
                                outputs, outputs2, outputs3, outputs4, sout_w, dout_w = model(inputs, inputs2, inputs3,
                                                                                              inputs4)
                ...
                if not opt.LPN and opt.circle:
                    ...
                    if opt.extra_Google:
                        ...
                        ff4 = F.normalize(ff4)
                        loss4 = criterion(logits4, labels4) + 2 * criterion_circle(
                    ...
                    if opt.views == 2:
                        ...
                        loss = criterion(outputs, labels) + 1 * criterion(outputs2, labels2) + criterion(outputs3,
                                                                                                         labels3)
                        if opt.extra_Google:
            ...
            if opt.views == 2:
                ...
                if opt.norm == 'ada-ibn' or opt.norm == 'spade':
                    if opt.style_loss:
                        ...
                        print(
                            '{} Loss: {:.4f} Satellite_Acc: {:.4f}  Street_Acc: {:.4f} Drone_Acc: {:.4f} WeatherLoss: {:.4f} W_S_Acc: {:.4f} W_D_Acc: {:.4f}'.format(
                                phase,
                                epoch_loss,
                                epoch_acc,
                                epoch_acc2,
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copyfile('./run_train.sh', dir_name + '/run_train.sh')
